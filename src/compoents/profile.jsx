...
export default function Profile() {
  ...
  },
  {
    ...
    return (
        <div style={{border: '1px solid black', padding: '10px', margin: '10px', display: 'grid',  textAlign: 'left'}}>
            <h2>All profile</h2>
                {data.map(actor => (
                    <Profilecard conf= {
                            <h5> confidential info </h5>
                    }
                    onClick={() => onclickevent(actor.name)}
                    onMouseEnter={onmoseenter}
                    key={actor.id} 
                    name={actor.name} 
                    language={actor.language} bio={actor.bio} 
